...
# --- 新增功能：用于设置Nginx订阅链接的函数 ---
setup_nginx_subscription(){
    read -p "请输入您已解析到本服务器IP的域名 (例如 sub.siffcity.sbs): " sub_domain
    if [[ -z "$sub_domain" ]]; then
        red "未输入域名，跳过创建永久订阅链接。"
        return
    fi

    # 检查Nginx是否已安装
    if ! command -v nginx &> /dev/null; then
        red "错误: Nginx 未安装。请先安装 Nginx 再创建订阅链接。"
        return
    fi
    ...
    mkdir -p "$SUB_DIR"
    mv /root/hy/clash_config.yaml "$SUB_DIR/clash.yaml" # 重命名为通用名称
    
    # 动态创建新的 server block 配置
    NEW_SERVER_BLOCK="
    # --- Hysteria2 Clash Subscription (Added by script) ---
    server {
        listen 443 ssl;
        listen [::]:443 ssl;
        http2 on;
...
"
    ...
    # 将新的 server block 追加到 http 块的末尾
    # 使用 awk 精确定位 http { ... } 的最后一个 '}' 之前插入
    awk -v new_block="$NEW_SERVER_BLOCK" '
        /http[[:space:]]*\{/ { in_http = 1 }
        in_http && /^\}/ { last_brace_line = NR }
        { print }
        END {
            if (last_brace_line) {
                system("sed -i \"" last_brace_line "i" new_block "\" " FILENAME)
            }
        }
    ' "$NGINX_CONF_PATH"
    ...
    if nginx -t &> /dev/null; then
        green "Nginx 配置测试通过，正在重载服务..."
        systemctl reload nginx
        CLASH_SUB_LINK="https://$sub_domain/clash.yaml"
    else
        red "错误: Nginx 配置测试失败！脚本已添加配置，但服务未重载。"
        red "请手动检查 $NGINX_CONF_PATH 文件并修复问题，然后运行 'systemctl reload nginx'。"
    fi
}
...
EOF
    ...
    # --- 新增功能：询问用户是否创建永久链接 ---
    echo ""
    read -p "是否需要创建一个永久的Clash订阅链接 (需要Nginx环境)? (y/N): " create_sub_link
    if [[ "$create_sub_link" =~ ^[yY]$ ]]; then
        setup_nginx_subscription
    fi
    # ----------------------------------------
    ...
    green "======================================================================================"
    green "已为 Clash Verge 生成专用配置文件，保存在 /var/www/clash_subscription/clash.yaml (如果创建了订阅)"
    yellow "您可以下载此文件或使用下面的永久订阅链接导入 Clash Verge！"
    
    if [[ -n "$CLASH_SUB_LINK" ]]; then
        green "您的永久Clash订阅链接如下："
        red "$CLASH_SUB_LINK"
    fi
    red "======================================================================================"
}
...
unsthysteria(){
    ...
    # 询问是否删除订阅相关文件和配置
    read -p "是否删除Nginx中的Clash订阅配置和文件? (y/N): " remove_sub_config
    if [[ "$remove_sub_config" =~ ^[yY]$ ]]; then
        # 简单实现：删除特定server块（需要用户手动确认域名）
        read -p "请输入您当时用于订阅的域名，脚本将尝试移除相关配置: " sub_domain_to_remove
        if [[ -n "$sub_domain_to_remove" ]]; then
            # 更安全的做法是注释掉而不是直接删除
            sed -i "/server_name $sub_domain_to_remove;/,/}/ s/^/#/" /etc/nginx/nginx.conf
            green "已尝试注释掉Nginx中关于 $sub_domain_to_remove 的配置。请手动检查并重载Nginx。"
        fi
        rm -rf /var/www/clash_subscription
        green "已删除 /var/www/clash_subscription 目录。"
    fi
...
generate_clash_config(){
    local clash_ip=$last_ip_no_brackets

    cat << EOF > /root/hy/clash_config.yaml
# Clash Verge 配置文件 - 由 Hysteria 2 脚本自动生成
